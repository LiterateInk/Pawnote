...
import decoders.decodeHomepage
...
suspend fun homepage (session: SessionHandle, day: LocalDateTime = session.instance.nextBusinessDay): Homepage {
    val weekNumber = translateToWeekNumber(day, session.instance.firstMonday)
    val next = encodePronoteDate(day)

    val request = RequestFN(session.information, "PageAccueil", Json.encodeToString(buildJsonObject {
        ...
        putJsonObject("donnees") {
            ...
            putJsonObject("coursNonAssures") {
                put("numeroSemaine", weekNumber)
            }

            putJsonObject("personnelsAbsents") {
                put("numeroSemaine", weekNumber)
            }

            putJsonObject("incidents") {
                put("numeroSemaine", weekNumber)
            }

            putJsonObject("exclusions") {
                put("numeroSemaine", weekNumber)
            }

            putJsonObject("donneesVS") {
                put("numeroSemaine", weekNumber)
            }
    ...
    return decodeHomepage(Json.decodeFromString<JsonObject>(response.data).jsonObject["donnees"]!!.jsonObject)
}