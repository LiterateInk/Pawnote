...
fun decodeNewDiscussionRecipient (recipient: JsonObject): NewDiscussionRecipient {
    ...
    if (recipient["listeRessources"] != null) {
        val sub = recipient["listeRessources"]!!.jsonObject["V"]!!.jsonArray
        ...
        for (resource in recipient["listeRessources"]!!.jsonObject["V"]!!.jsonArray) {
            ...
            subjects.add(decodeNewDiscussionRecipientSubject(resource.jsonObject, sub.filter { it.from == resource.jsonObject["L"]!!.jsonPrimitive.content }))
        }
    }
    ...
    return NewDiscussionRecipient(
        ...
        subjects = subjects,
        function = if (recipient["fonction"] != null) decodeNewDiscussionRecipientFunction(recipient["fonction"]!!.jsonObject["V"]!!.jsonObject) else null,
