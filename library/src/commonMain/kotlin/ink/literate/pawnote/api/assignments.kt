...
val decoder: (sessionInformation: SessionInformation, data: JsonObject) -> List<Assignment> =
    { sessionInformation, data ->
      data["ListeTravauxAFaire"]!!.jsonObject["V"]!!.jsonArray.map {
        decodeAssignment(it.jsonObject, sessionInformation)
      }
    }

suspend fun assignmentsFromWeek(
    sessionInformation: SessionInformation,
    weekNumber: Int,
    extendsToWeekNumber: Int? = null
): List<Assignment> {
  val reply =
      homeworkFromWeek(sessionInformation, TabLocation.Assignments, weekNumber, extendsToWeekNumber)
  return decoder(sessionInformation, reply)
}

suspend fun assignmentsFromIntervals(
