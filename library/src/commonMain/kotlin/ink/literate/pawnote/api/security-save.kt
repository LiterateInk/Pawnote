...
suspend fun securitySave(session: SessionHandle, handle: SecurityModal, options: SecSaveOptions) {
          ...
          JsonObject(
              data +
                  buildJsonObject {
                    put(
                        "nouveauMDP",
                        AES.encrypt(options.password.toByteArray(), keys.key, keys.iv))
                  })
  ...
  if (options.pin != null)
      data =
          JsonObject(
              data +
                  buildJsonObject {
                    put("codePin", AES.encrypt(options.pin.toByteArray(), keys.key, keys.iv))
                  })
  ...
  val request =
      RequestFN(
          ...
          Json.encodeToString(buildJsonObject { put("donnees", data) }))

  val response = request.send()
  val token =
      Json.parseToJsonElement(response.data)
          .jsonObject["donnees"]!!
          .jsonObject["jetonConnexionAppliMobile"]
          ?.jsonPrimitive
          ?.content
  ...
  if (token != null)
      handle.context.authentication =
          JsonObject(
              handle.context.authentication +
                  buildJsonObject { put("jetonConnexionAppliMobile", token) })
}
