...
import ink.literate.pawnote.models.SessionAccessKind
...
fun decodeSessionInformation(session: JsonObject, base: String): SessionInformation {
  ...
  return SessionInformation(
      ...
      accessKind =
          if (session["g"] != null) SessionAccessKind.fromInt(session["g"]!!.jsonPrimitive.int)
          else SessionAccessKind.ACCOUNT,
      rsaModulus = if (rsaFromConstants) rsaModulo1024 else session["MR"]!!.jsonPrimitive.content,
      rsaExponent =
          if (rsaFromConstants) rsaExponent1024 else session["ER"]!!.jsonPrimitive.content,
