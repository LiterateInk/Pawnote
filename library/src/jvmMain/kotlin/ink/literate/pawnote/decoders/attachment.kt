...
actual fun decodeAttachment(
  ...
  if (kind == AttachmentKind.Link.code) url = attachment["url"]?.jsonPrimitive?.content ?: name
  else {
    ...
    url =
        "${sessionInfo.url}/FichiersExternes/${encrypted}/${URLEncoder.encode(name)}?Session=${sessionInfo.id}"
  }

  return Attachment(kind = AttachmentKind.fromInt(kind), name = name, url = url, id = id)
}
