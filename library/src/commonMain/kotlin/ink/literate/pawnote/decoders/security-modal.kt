...
fun decodeSecurityModal (authentication: JsonObject, identity: IdentifyResponse, initialUsername: String? = null): SecurityModal {
    val actions = ink.literate.pawnote.decoders.decodeDomain(authentication["actionsDoubleAuth"]!!.jsonObject["V"]!!.jsonPrimitive.content)
        .map { DoubleAuthClientAction.fromInt(it) }

    return SecurityModal(
        availableSecurityModes = ink.literate.pawnote.decoders.decodeDomain(authentication["modesPossibles"]!!.jsonObject["V"]!!.jsonPrimitive.content)
        ...
        passwordRules = ink.literate.pawnote.decoders.decodePasswordRules(authentication["reglesSaisieMDP"]!!.jsonObject),
