...
import models.TimetableClassLesson
...
fun decodeTimetableClassLesson (item: JsonObject): TimetableClassLesson {
    ...
    if (item.containsKey("listeVisios")) {
        for (virtualClassroom in item["listeVisios"]!!.jsonObject["V"]!!.jsonArray)
            virtualClassrooms.add(virtualClassroom.jsonObject["url"]!!.jsonPrimitive.content)
    }
    ...
    if (item["AvecCdT"]?.jsonPrimitive?.boolean == true && item.containsKey("cahierDeTextes"))
        lessonResourceID = item["cahierDeTextes"]!!.jsonObject["V"]!!.jsonObject["N"]!!.jsonPrimitive.content
    ...
    return TimetableClassLesson(
        ...
        test = if (item["cahierDeTextes"] != null) item["cahierDeTextes"]!!.jsonObject["V"]!!.jsonObject["estDevoir"]?.jsonPrimitive?.boolean ?: false else false,
