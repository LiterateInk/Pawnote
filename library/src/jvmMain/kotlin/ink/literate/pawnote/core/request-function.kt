...
import ink.literate.pawnote.api.private.Keys
...
actual class Payload actual constructor(actual val order: String, actual val url: Url)

actual class RequestFN
actual constructor(
  ...
  actual fun process(): Payload {
    ...
    val url =
        Url(
            "${this.sessionInfo.url}/appelfonction/${this.sessionInfo.accountKind.code}/${this.sessionInfo.id}/${order}")

    if (!this.sessionInfo.skipCompression) this.compress()

    if (!this.sessionInfo.skipEncryption) this.encrypt()
  ...
  private fun encrypt() {
    ...
    else this.data = AES.encrypt(this.data.toByteArray(), keys.key, keys.iv)
  }
