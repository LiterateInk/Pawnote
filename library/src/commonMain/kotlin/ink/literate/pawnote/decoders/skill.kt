...
fun decodeSkill (skill: JsonObject) = Skill(
    ...
    itemID = if (skill["item"] != null) skill["item"]!!.jsonObject["V"]!!.jsonObject["N"]!!.jsonPrimitive.content else null,
    itemName = if (skill["item"] != null) skill["item"]!!.jsonObject["V"]!!.jsonObject["L"]!!.jsonPrimitive.content else null,
    ...
    pillarPrefixes = skill["pilier"]!!.jsonObject["V"]!!.jsonObject["strPrefixes"]!!.jsonPrimitive.content.split(',').map { it.trim() }
)