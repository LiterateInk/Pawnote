...
import ink.literate.pawnote.models.UserAuthorizations
...
fun decodeUserAuthorizations(data: JsonObject, tabs: JsonArray): UserAuthorizations {
  ...
  val canDiscuss =
      canReadDiscussions && !(data["discussionInterdit"]?.jsonPrimitive?.boolean ?: false)
  val canDiscussWithStaff =
      canDiscuss && !(data["AvecDiscussionPersonnels"]?.jsonPrimitive?.boolean ?: false)
  val canDiscussWithParents =
      canDiscuss && !(data["AvecDiscussionParents"]?.jsonPrimitive?.boolean ?: false)
  val canDiscussWithStudents =
      canDiscuss && !(data["AvecDiscussionEleves"]?.jsonPrimitive?.boolean ?: false)
  val canDiscussWithTeachers =
      canDiscuss && !(data["AvecDiscussionProfesseurs"]?.jsonPrimitive?.boolean ?: false)
  ...
  if (tabs.isNotEmpty()) {
    fun traverse(obj: JsonObject): Unit = run {
      ...
      if (obj.containsKey("Onglet")) obj["Onglet"]!!.jsonArray.forEach { traverse(it.jsonObject) }
    }
