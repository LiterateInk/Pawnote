...
import ink.literate.pawnote.models.errors.*
...
actual constructor(
    ...
    try {
      ...
      if (this.jsonData.jsonObject.containsKey("_Signature_") &&
          ...
          throw ServerSideError(
              this.jsonData.jsonObject["_Signature_"]!!
                  .jsonObject["MessageErreur"]!!
                  .jsonPrimitive
                  .content)
    } catch (error: Exception) {
      if (content.contains("La page a expir")) throw SessionExpiredError()
      else if (content.contains("Votre adresse IP ")) throw SuspendedIPError()
      else if (content.contains("La page dem") || content.contains("Impossible d'a"))
          throw PageUnavailableError()
      else if (content.contains("Vous avez d")) throw RateLimitedError()
  ...
  private fun decrypt() {
    ...
    val decryptedData =
        AES.decrypt(
            Json.parseToJsonElement(this.data).jsonPrimitive.content.decodeHex().toByteArray(),
            key,
            iv)

    if (!this.sessionInfo.skipCompression) this.data = decryptedData.toHexString()
    else this.data = decryptedData.decodeToString()
  }
