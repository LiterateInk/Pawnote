...
fun decodeResourceContent(content: JsonObject, session: SessionHandle) =
    ResourceContent(
        ...
        category =
            ResourceContentCategory.fromInt(
                content["categorie"]!!.jsonObject["V"]!!.jsonObject["G"]!!.jsonPrimitive.int),
        files =
            content["ListePieceJointe"]!!.jsonObject["V"]!!.jsonArray.map {
              decodeAttachment(it.jsonObject, session.information)
            },
        themes =
            content["ListeThemes"]!!.jsonObject["V"]!!.jsonArray.map {
