...
class RequestUpload(private val session: SessionHandle, functionName: String, file: File, private val fileName: String) {
    ...
    init {
        ...
        this.form = form
        this.url = session.information.url + "/uploadfilesession/${session.information.accountKind.code}/${session.information.id}"
    }
    ...
    suspend fun send() {
        ...
        while (state == 3) {
            val response = this.session.client.submitFormWithBinaryData(url = url,form!!) {
                headers {
