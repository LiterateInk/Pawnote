...
suspend fun assignmentUploadFile (session: SessionHandle, assignmentID: String, file: File, fileName: String) {
    ...
    if (fileSize > maxFileSize)
        throw UploadSizeError(maxFileSize)

    // Ask to the server to store the file for us.
    val fileUpload = RequestUpload(session, "SaisieTAFARendreEleve", file, fileName)
    fileUpload.send()

    val request = RequestFN(session.information, "SaisieTAFARendreEleve", Json.encodeToString(
        buildJsonObject {
