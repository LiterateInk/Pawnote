...
val baseParams = mapOf("fd" to "1", "login" to "true")

val hasSecurityModal: (authentication: JsonObject) -> Boolean = {
  it["actionsDoubleAuth"]?.jsonPrimitive?.boolean ?: false
}

fun transformCredentials(
  ...
  if (identity.modeCompMdp == 1)
      return when (modProperty) {
        ModProperty.password ->
            auth.copy(username = username, password = auth.password!!.lowercase())
...
fun createMiddlewareKey(identity: IdentifyResponse, username: String, mod: String): ByteArray {
  ...
  val hash =
      md.digest("${identity.alea ?: ""}${mod}".toByteArray()).toHexString(HexFormat.UpperCase)
...
fun solveChallenge(
  ...
  val bytes =
      AES.decrypt(identity.challenge.hexToByteArray(HexFormat.UpperCase), key, iv).decodeToString()
...
fun switchToAuthKey(
  ...
  val iv = sessionInfo.aesIV.hexToByteArray()
  val authKeyDecrypted =
      AES.decrypt(authentication["cle"]!!.jsonPrimitive.content.hexToByteArray(), key, iv)
  val authKey =
      authKeyDecrypted
          .decodeToString()
          .split(",")
          .map { char -> char.toInt().toByte() }
          .toByteArray()
          .toHexString()
...
suspend fun loginCredentials(auth: CredentialsAuth): LoginResult {
  ...
  val sessionInfo =
      sessionInformation(
          SessionInfoParams(
              ...
              kind = auth.kind,
              cookies = listOf(),
              params = baseParams.plus("bydlg" to "A6ABB224-12DD-4E31-AD3E-8A39A1C2C335")))
  ...
  val key =
      createMiddlewareKey(
          identity, transformedCredentials.username, transformedCredentials.password!!)
  ...
  if (hasSecurityModal(authentication)) return switchToTokenLogin(auth)

  return finishLoginManually(
      base, finalSessionInfo, sessionInstance, authentication, identity, auth.username)
}
...
suspend fun loginToken(auth: CredentialsAuth): LoginResult {
  ...
  val key =
      createMiddlewareKey(identity, transformedCredentials.username, transformedCredentials.token!!)

  val challenge = solveChallenge(sessionInfo, identity, key)
  val authentication = authenticate(sessionInfo, challenge)
  val finalSessionInfo = switchToAuthKey(sessionInfo, authentication, key)

  if (hasSecurityModal(authentication)) throw SecurityError(authentication, identity, auth.username)
...
suspend fun loginQrCode(info: ink.literate.pawnote.models.QrInfo): LoginResult {
  ...
  if (hasSecurityModal(authentication)) return switchToTokenLogin(transformedCredentials)
  else
      return finishLoginManually(
          qr.url, finalSessionInfo, sessionInstance, authentication, identity, auth.username)
}
