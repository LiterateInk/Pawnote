...
import models.NotebookAbsence

fun decodeNotebookAbsence (absence: JsonObject): NotebookAbsence {
    val (hoursMissed, minutesMissed) = absence["NbrHeures"]!!.jsonPrimitive.content.split('h').map { it.toInt() }
    ...
    return NotebookAbsence(
        ...
        reason = if (!isReasonUnknown) absence["listeMotifs"]!!.jsonObject["V"]!!.jsonArray[0].jsonObject["L"]!!.jsonPrimitive.content else null
    )
}