...
) : Exception("You're asked to custom your security methods") {
  val handle: SecurityModal =
      ink.literate.pawnote.decoders.decodeSecurityModal(authentication, identity, initialUsername)
}

class SecuritySourceTooLongError(limit: Int) :
    Exception("The source name is too long, limited to $limit characters")
